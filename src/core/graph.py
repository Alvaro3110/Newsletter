...
"""
    ...
    while len(essay_body.split()) < 1000:
        essay_body += """

Como reforço operacional, a decisão deve ser acompanhada por um limite explícito de escopo: usar SQLite apenas quando a escrita concorrente estiver controlada por uma única fila de trabalho, com timeouts mensurados, logs de erro persistidos e restauração testada em pelo menos 3 execuções repetidas. Esse critério evita que a simplicidade local esconda uma dependência de coordenação que deveria estar em Postgres, Redis ou outra camada distribuída.
"""
